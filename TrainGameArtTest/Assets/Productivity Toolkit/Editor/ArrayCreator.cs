...
namespace ToolExtensions
{
    ...
    public class ArrayCreator : EditorWindow
    {
        ...
        private Vector3Int _boundsAmount;
        ...
        private string[] _incrementalTotalBounds = new string[] { "Incremental", "Total","Bound to bound" };
        ...
        private void OnGUI()
        {
            ...
            _incrementalTotalMoveIndex = EditorGUILayout.Popup(_incrementalTotalMoveIndex, _incrementalTotalBounds);
            GUILayout.Space(10);
        ...
        private GameObject ObjectInstantiate()
        {
            ...
            else
            {
                ...
                instantiatedObject.transform.localScale = _objectToArray.transform.localScale;
            }
        ...
        {
            int indexchange =  ignoreFirstInstance? 0 : 1;
            for (int i = 1 - indexchange; i < count; i++)
            {
                GameObject instantiatedObject = ObjectInstantiate();
                instantiatedObject.name = _objectToArray.name + "_" + string.Format("{0:00}", i);

                instantiatedObject.transform.rotation = _incrementalTotalrotateIndex == 0 ? Quaternion.Euler(_rotationTransformation * i + _objectToArray.transform.rotation.eulerAngles) : Quaternion.Euler((_rotationTransformation / _oneDimensionCount) * i + _objectToArray.transform.rotation.eulerAngles);
                instantiatedObject.transform.position = _objectToArray.transform.position;
                if (_incrementalTotalMoveIndex < 2) // Incremental and total
                {
                ...
                {
                    instantiatedObject.transform.Translate( Vector3.Scale(rowOffset + _moveTransformation, _objectToArray.GetComponentInChildren<MeshFilter>().sharedMesh.bounds.size)  * i);
                }
